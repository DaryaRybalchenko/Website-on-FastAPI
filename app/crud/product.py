...
from app.models.writeoff_acts import WriteOffActItem
...
def get_products(db: Session, skip: int = 0, limit: int = 100):
    return (
        ...
        .order_by(models.Product.id)  # сортировка по возрастанию id
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def delete_product(db: Session, product_id: int):  
    try:
        ...
        # Удаляем связанные записи в writeoff_act_items
        db.query(WriteOffActItem).filter(WriteOffActItem.product_id == product_id).delete()
